    ...
    **IRON SANDBOX PROTOCOL**:
    ...
    try:
                ...
                for t in entry["tactics"]:
                    ...
                    if "cleavages" in entry:
                        for i, score in enumerate(entry["cleavages"]):
                            if score > 0.5:
                                cleav_id = f"c_{i}" 
                                if not G.has_node(cleav_id):
                                    G.add_node(cleav_id, label=f"Cleavage {i}", type="cleavage")
                                
                                # Add or update edge weight
                                if G.has_edge(t, cleav_id):
                                    G[t][cleav_id]['weight'] += score
                                else:
        ...
        for node_id, (x, y) in pos.items():
            ...
            color = "#2563eb" if node_type == "tactic" else "#db2777"
            
            output_nodes.append({
                "id": str(node_id),
                "position": {"x": (x+1) * 400, "y": (y+1) * 300},
        ...
        # Return empty graph on error to prevent UI crash
